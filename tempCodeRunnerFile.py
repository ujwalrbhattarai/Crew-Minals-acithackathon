...
while cap.isOpened():
    ...
    if results.multi_face_landmarks:
        ...
        if not calibrated:
            ...
            # Calculate head angle relative to both horizontal and vertical axes
            horizontal_angle = np.degrees(np.arctan2(eye_vector[1], eye_vector[0]))
            vertical_angle = np.degrees(np.arctan2(nose_y - (left_eye[1] + right_eye[1]) / 2, nose_x - (left_eye[0] + right_eye[0]) / 2))

            # Combine horizontal and vertical angles for violation detection
            if abs(horizontal_angle) > HEAD_ANGLE_TOL or abs(vertical_angle) > HEAD_ANGLE_TOL:
                outside_head_frame_count += 1
                if outside_head_frame_count >= OUTSIDE_FRAMES_REQUIRED:
                    head_violation_counter += 1
                    outside_head_frame_count = 0
            ...
            # Ensure head bounding box variables are defined before use
            ...
            # Display both angles for debugging
            cv2.putText(frame, f"Horizontal Angle: {horizontal_angle:.2f} deg", (50, 140),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1)
            cv2.putText(frame, f"Vertical Angle: {vertical_angle:.2f} deg", (50, 160),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1)
